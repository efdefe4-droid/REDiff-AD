...
def test(obj_names, mvtec_path, checkpoint_path, split='all', result_dir=None):
    obj_ap_pixel_list = []
    ...
    for obj_name in obj_names:
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            result.save(os.path.join(result_dir, obj_name + '_' + str(i_batch) + "_mask.jpg"))
            # 
        ...
        best_threshold = thresholds[min(best_f1_score_index, len(thresholds) - 1)] if len(thresholds) else 0.0
...
if __name__=="__main__":
    ...
    parser.add_argument('--result_dir', type=str, default=os.path.join(os.path.dirname(__file__), 'test_results'))
    ...
    with torch.cuda.device(args.gpu_id):
        test(obj_list,args.mvtec_path, args.checkpoint_path, split=args.split, result_dir=args.result_dir)
